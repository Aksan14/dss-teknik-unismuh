        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg shadow-md p-6 mb-6">
          <div className="flex flex-wrap gap-4 justify-between items-center">
            <p className="text-purple-900 font-semibold">
              Total Alumni: <span className="text-2xl text-purple-600">{loading ? '-' : filteredData.length.toLocaleString()}</span>
            </p>
            {totalPages > 1 && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-purple-100 border-b border-gray-200">
                  ...
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">IPK</th>
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">SKS Lulus</th>
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">SKS Diambil</th>
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">MK Lulus</th>
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">MK Diulang</th>
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">SKS MK Diulang</th>
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">Kategori</th>
                  <th className="px-6 py-4 text-left font-semibold text-purple-900">Aksi</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={13} className="px-6 py-8 text-center text-gray-500">
                      {search ? 'Tidak ada data yang cocok' : 'Tidak ada data alumni'}
                    </td>
                  </tr>
                ) : (
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {getPageNumbers().map((page) => (
                    <button key={page} onClick={() => setCurrentPage(page)} disabled={loading} className={`px-3 py-1 rounded-lg border ${page === currentPage ? 'bg-purple-500 text-white border-purple-500' : 'border-gray-300 hover:bg-gray-100'} disabled:opacity-50`}>
                      {page}
                    </button>
                  ))}
