...
export default function DataLengkap() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:8080/api/v1/mahasiswa?limit=10000');
      ...
      <header className="bg-white shadow-md border-b border-indigo-100">
        <div className="px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-indigo-900">Data Lengkap Mahasiswa</h1>
              <p className="text-sm text-indigo-600">Seluruh data mahasiswa Fakultas Teknik</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="bg-indigo-50 border border-indigo-200 rounded-lg shadow-md p-6 mb-6">
          <div className="flex flex-wrap gap-4 justify-between items-center">
            <p className="text-indigo-900 font-semibold">
              Total Mahasiswa: <span className="text-2xl text-indigo-600">{loading ? '-' : filteredData.length.toLocaleString()}</span>
            </p>
            {totalPages > 1 && (
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-indigo-100 border-b border-gray-200">
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">No</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">Nama</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">NIM</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">Angkatan</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">IPK</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">SKS Lulus</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">SKS Diambil</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">MK Lulus</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">MK Diulang</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">SKS MK Diulang</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">Status</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">Kategori</th>
                  <th className="px-6 py-4 text-left font-semibold text-indigo-900">Aksi</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {getPageNumbers().map((page) => (
                    <button key={page} onClick={() => setCurrentPage(page)} disabled={loading} className={`px-3 py-1 rounded-lg border ${page === currentPage ? 'bg-indigo-500 text-white border-indigo-500' : 'border-gray-300 hover:bg-gray-100'} disabled:opacity-50`}>
                      {page}
                    </button>
                  ))}
