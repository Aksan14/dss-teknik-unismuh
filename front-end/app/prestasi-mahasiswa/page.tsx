...
export default function PrestasiMahasiswa() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-md border-b border-yellow-100">
        <div className="px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center space-x-3">
            <HomeIcon className="h-8 w-8 text-yellow-600" />
            <div>
              <h1 className="text-2xl font-bold text-yellow-900">Mahasiswa Berprestasi</h1>
              <p className="text-sm text-yellow-600">Data mahasiswa dengan IPK tinggi (≥ 3.5)</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg shadow-md p-6 mb-6">
          <div className="flex flex-wrap gap-4 justify-between items-center">
            <p className="text-yellow-900 font-semibold">
              Total Mahasiswa Berprestasi: <span className="text-2xl text-yellow-600">{loading ? '-' : filteredData.length.toLocaleString()}</span>
            </p>
            {totalPages > 1 && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-yellow-100 border-b border-gray-200">
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">Ranking</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">Nama</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">NIM</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">Angkatan</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">Jurusan</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">IPK</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">SKS Lulus</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">SKS Diambil</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">MK Lulus</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">MK Diulang</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">SKS MK Diulang</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">Status</th>
                  <th className="px-6 py-4 text-left font-semibold text-yellow-900">Aksi</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={13} className="px-6 py-8 text-center text-gray-500">
                      {search ? 'Tidak ada data yang cocok' : 'Tidak ada mahasiswa berprestasi'}
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={mahasiswa.nim} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          (currentPage - 1) * PER_PAGE + idx < 3 ? 'bg-yellow-400 text-yellow-900' : 'bg-gray-100 text-gray-800'
                        }`}>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {getPageNumbers().map((page) => (
                    <button key={page} onClick={() => setCurrentPage(page)} disabled={loading} className={`px-3 py-1 rounded-lg border ${page === currentPage ? 'bg-yellow-500 text-white border-yellow-500' : 'border-gray-300 hover:bg-gray-100'} disabled:opacity-50`}>
                      {page}
                    </button>
                  ))}
