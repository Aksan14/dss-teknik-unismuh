        ...
        <div className="bg-red-50 border border-red-200 rounded-lg shadow-md p-6 mb-6">
          <div className="flex flex-wrap gap-4 justify-between items-center">
            <p className="text-red-900 font-semibold">
              Total Mahasiswa Tidak Aktif: <span className="text-2xl text-red-600">{loading ? '-' : filteredData.length.toLocaleString()}</span>
            </p>
            {totalPages > 1 && (
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {getPageNumbers().map((page) => (
                    <button key={page} onClick={() => setCurrentPage(page)} disabled={loading} className={`px-3 py-1 rounded-lg border ${page === currentPage ? 'bg-red-600 text-white border-red-600' : 'border-gray-300 hover:bg-gray-100'} disabled:opacity-50`}>
                      {page}
                    </button>
                  ))}
